...
HOUR_START = 8
HOUR_END = 19
SCHOOL_START = dt.date(2025, 9, 29)  # Okulun açıldığı tarih
...
def auto_push():
    ...
    if not repo_url or not token:
        st.warning("GitHub repo URL veya token eksik, değişiklikler pushlanamayacak.")
        return
    try:
        # Git URL'ine token ekle
        auth_url = repo_url.replace("https://", f"https://{token}@")
        subprocess.run(["git", "add", "."], check=True)
        subprocess.run(["git", "commit", "-m", "Auto-update attendance/schedule"], check=True)
        subprocess.run(["git", "push", auth_url, "HEAD:main"], check=True)
    except subprocess.CalledProcessError:
        st.warning("GitHub push sırasında hata oluştu.")
...
def timetable_grid_figure(schedule_df: pd.DataFrame, title: str = "Haftalık Program"):
    ...
    for _, r in schedule_df.iterrows():
        ...
        fig.add_shape(
            type="rect",
            x0=x-0.45, x1=x+0.45, y0=y0, y1=y1,
...
def dashboard(username: str):
    ...
    # -------- Katılım İşaretle --------
        ...
        if my_sched.empty:
            st.info("Önce ders ekleyin."); return
        fig = timetable_grid_figure(my_sched,"Haftalık Program")
        st.plotly_chart(fig,use_container_width=True)
        sel_date = st.date_input("Tarih", value=dt.date.today())
        weekday = sel_date.weekday()
        if weekday>4: st.info("Hafta içi program gösteriliyor.")
        ...
        if todays.empty: st.info(f"{weekday_tr} günü ders yok.")
            ...
            for _,r in todays.iterrows():
                ...
                if checked and not already:
                    ...
                    attendance_df = attendance_df[~((attendance_df["username"]==username)&(attendance_df["course"]==r["course"])&(attendance_df["date"]==sel_date))]
                    save_attendance(attendance_df)
    ...
    # -------- İstatistikler --------
        ...
        if my_att.empty:
            ...
            return
        # Bugüne kadar derslerin toplam sayısı
        all_sched = schedule_df[schedule_df["username"]==username].copy()
        if all_sched.empty:
            st.info("Önce program ekleyin."); return
        today = dt.date.today()
        # bugüne kadar geçerli dersler
        valid_days = [d for d in DAYS_TR if DAY_IDX[d]<=today.weekday()]
        valid_sched = all_sched[all_sched["day"].isin(valid_days)]
        total_lessons = len(valid_sched)
        attended = len(my_att[my_att["date"]<=today])
    ...
    # -------- Sıralama --------
        ...
        if all_sched.empty or all_att.empty:
            st.info("Henüz veri yok."); return
        today = dt.date.today()
        results=[]
        for user in all_sched["username"].unique():
            user_sched = all_sched[all_sched["username"]==user]
            user_att = all_att[all_att["username"]==user]
            # bugüne kadar toplam ders
            valid_days = [d for d in DAYS_TR if DAY_IDX[d]<=today.weekday()]
            valid_sched = user_sched[user_sched["day"].isin(valid_days)]
            total_lessons = len(valid_sched)
            attended = len(user_att[user_att["date"]<=today])
            perc = (attended/total_lessons*100) if total_lessons>0 else 0
