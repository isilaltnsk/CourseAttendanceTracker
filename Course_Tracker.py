...
HOUR_START = 8
HOUR_END = 19
SCHOOL_START = dt.date(2025, 9, 29)  # okul başlangıç tarihi
...
def timetable_grid_figure(schedule_df: pd.DataFrame, title: str = "Haftalık Program"):
    ...
    fig.update_layout(
        ...
        yaxis=dict(range=[HOUR_END, HOUR_START],
                   tick0=HOUR_START,
                   dtick=1,
                   tickformat="02d:00",
    ...
    for _, r in schedule_df.iterrows():
        ...
        fig.add_annotation(x=x, y=(y0+y1)/2,
...
# ================== Git Push ==================
def git_push():
    """Localde kaydedilen CSV dosyalarını GitHub'a push eder."""
    try:
        repo_url = st.secrets["MY_GITHUB_REPO_URL"]
        token = st.secrets["MY_GITHUB_TOKEN"]
        auth_url = repo_url.replace("https://", f"https://{token}@")
        subprocess.run(["git", "add", "."], check=True)
        subprocess.run(["git", "commit", "-m", "Auto-save from Streamlit app"], check=True)
        subprocess.run(["git", "push", auth_url, "main"], check=True)
    except Exception as e:
        st.warning("GitHub push sırasında hata oluştu, değişiklikler local olarak kaydedildi.")
...
def dashboard(username: str):
    ...
    if choice == "Ders Programı":
        ...
        if submitted:
            if end <= start:
                ...
                row = pd.DataFrame([{"username": username, "course": course.strip(),
                                     "day": day, "start": start.strftime("%H:%M"),
    ...
    # -------- İstatistikler --------
        ...
        courses_before_today = my_sched[(my_sched["day"].apply(lambda d: DAY_IDX[d] <= dt.date.today().weekday()))]
        total_possible = len(courses_before_today)
        actual_attended = len(my_att[my_att["date"] <= dt.date.today()])

        perc = (actual_attended / total_possible * 100) if total_possible > 0 else 0

        summary = my_att.groupby("course").date.nunique().reset_index(name="Katılım Sayısı")
        st.dataframe(summary, use_container_width=True)
        st.metric("Toplam Katılım", int(actual_attended))
    ...
    # -------- Sıralama --------
        ...
        for user in all_sched["username"].unique():
            ...
            # bugüne kadar ders sayısı
            courses_before_today = user_sched
